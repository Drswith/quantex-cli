...
- [x] 5.6 Diagnose with release-please's own dry run instead of guessing again. It reproduced `updating from 1.10.0 to 2.0.0` and revealed the real cause: the changelog entries for the delivery commits carry the text of the pull request's `BEGIN_COMMIT_OVERRIDE` block, not the commit subjects. Release-please replaces the merged commit message with that block, and the `Release-As` footer was placed after `END_COMMIT_OVERRIDE`, so the message it parsed never contained the override. The earlier line-wrap theory was wrong.
- [x] 5.7 Require the `Release-As` footer inside the override block in `pr-body-policy`, cover it in `test/pr-body-policy.test.ts`, correct the pre-existing test that encoded the wrong placement, and record the rule in `release-workflow` and the release runbook.
- [x] 5.8 Re-trigger the boundary with the footer inside the override block. The delivery commit for 5.7 destroyed itself instead: its message explained the mechanism by naming the override markers literally, so release-please parsed the sentence fragment between them, logged `commit could not be parsed`, and dropped the commit from the release with its footer. The dry run showed exact correlation — one unparseable commit in the range, one commit containing the markers, the same commit.
- [x] 5.9 Reject stray override markers in both `commit-policy` and `pr-body-policy`, cover both in tests, and record the rule in `release-workflow` and the release runbook.
- [x] 5.10 Stop verifying by merging. Add `release:verify-release-as`, which runs release-please's own `parseConventionalCommits` over the pull request body and reports the version it would actually release. Pin `release-please` as a devDependency so the check is offline and deterministic.
- [x] 5.11 Capture the real bodies of PRs #670 to #673 as fixtures and assert each failure mode in `test/release-as-verification.test.ts`: footer outside the block yields no version, marker words in the message drop the commit entirely, and the corrected shape yields `1.11.0`.
- [x] 5.12 Run the verifier in the governance job so no future declaration can merge unverified.
- [x] 5.13 Confirm locally, before merging, that the current delivery body would release `1.11.0`.
- [ ] 5.14 Confirm Release Please replaces the `2.0.0` Release PR with `1.11.0` and that `governance` passes on it.
- [ ] 5.15 Merge the Release PR, confirm `tag-release` tags `v1.11.0` and `release.yml` publishes, then verify the published package accepts `qtx update --all --managed` and `qtx update --non-interactive`.
- [ ] 5.16 Report validation, OpenSpec, git, commit, remote, PR, release, and archive-closure state.
