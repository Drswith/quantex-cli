...
- [ ] 5.8 Re-trigger the boundary with the footer inside the override block.
- [ ] 5.9 Confirm Release Please replaces the `2.0.0` Release PR with `1.11.0` and that `governance` passes on it.
- [ ] 5.10 Merge the Release PR, confirm `tag-release` tags `v1.11.0` and `release.yml` publishes, then verify the published package accepts `qtx update --all --managed` and `qtx update --non-interactive`.
- [ ] 5.11 Report validation, OpenSpec, git, commit, remote, PR, release, and archive-closure state.
