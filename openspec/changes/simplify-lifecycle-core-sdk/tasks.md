...
- [ ] 2.1 Include the integration branch in CI, release-verification, and Sandbox trigger classification without widening protected release targets
- [ ] 2.2 Make Linux, macOS, and Windows validation explicit for Core routing promotion and stop treating skipped Windows PR coverage as sufficient
- [ ] 2.3 Add semantic compile coverage for every maintained root export before changing declaration generation; keep the existing v1 fixtures unchanged
...
- [ ] 3.1 Add the single `packages/core` workspace, exact same-version root development dependency, Node-only TypeScript configuration, and explicit isolated Core build
- [ ] 3.2 Implement the minimal public types for `createQuantex`, `CoreResult`, request options, agent descriptors, and the managed/external/missing/stale/conflict/indeterminate inspection union
- [ ] 3.3 Implement instance-owned `list` and `inspect` without Commander, prompts, presenters, mutable CLI context, writes, process exit, or self-upgrade imports
- [ ] 3.4 Make the maintained CLI inspection paths consume the canonical Core read implementation while preserving current command, JSON/NDJSON, error, exit, and stream fixtures
- [ ] 3.5 Add Core re-entrancy, cancellation isolation, timeout, corrupt-state, no-output, no-write, and no-network-on-import tests
- [ ] 3.6 Add an import-boundary test that rejects Core dependencies on CLI commands, presenters, mutable CLI context, self-upgrade, release artifacts, and unsupported public subpaths
- [ ] 3.7 Add clean packed-tarball Node.js 20 ESM, Bun, and TypeScript NodeNext consumers and verify the Core artifact allowlist
- [ ] 3.8 Prove the root CLI tarball and standalone binaries inline Core, retain the v1 root contract, and run without Core installed or repository `node_modules`
- [ ] 3.9 Commit and push the working read-only SDK vertical slice as the second recoverable checkpoint

## 4. Coordinated packaging and release recovery

- [ ] 4.1 Synchronize root and Core versions through one release-please component and reject mismatched title, root, Core, or exact Core development-dependency versions
- [ ] 4.2 Reject workspace protocols from publishable manifests and keep the external `quantex` alias outside this repository's release coordination
- [ ] 4.3 Extend package validation and release resolution to represent Core-missing, CLI-missing, both-missing, both-published, and registry-indeterminate states
- [ ] 4.4 Publish and verify Core before CLI, preserve idempotent per-package recovery, and upload standalone artifacts only after repository npm closure
- [ ] 4.5 Pin release workflow Bun versions to the repository toolchain and include Core manifests in cache keys, release PR policy, path taxonomy, and Sandbox selection
...
- [ ] 8.1 Add an ADR for the two-package boundary, frozen 1.x state shape, engine-selection rule, and release train
- [ ] 8.2 Document current SDK installation, runtime requirements, supported methods, CLI/Core responsibility split, compatibility stage, and rollback boundary in English and Simplified Chinese
- [ ] 8.3 Run `bun install --frozen-lockfile`, lint, format check, root and Core typecheck, tests, OpenSpec validation, memory check, both builds, package checks, standalone binary build, and release artifact validation
