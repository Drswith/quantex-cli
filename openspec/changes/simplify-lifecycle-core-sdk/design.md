...
- [An older release commit lacks a recovery validator introduced later] → Separate immutable release source from the validated protected-branch control snapshot, prefer the target-owned harness when present, and use the control harness only for pre-harness historical recovery.
