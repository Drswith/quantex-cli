...
The agent update system SHALL recognize Bun global untrusted package output for requested managed packages regardless of whether Bun prints `node_modules` paths with POSIX or Windows separators. When the untrusted probe cannot be read after a successful Bun global install or update command, Quantex SHALL NOT report that managed operation as successful. When trust verification fails after a successful Bun global **install** command, Quantex SHALL roll back only packages that were absent from Bun globals before `bun add -g`, then report install failure.
...
- AND those packages were absent from Bun globals before `bun add -g`
...
#### Scenario: Preserving an already Bun-global package when trust fails after re-add

- GIVEN a Bun-managed install requested a package that was already present in Bun globals
- AND `bun add -g` exits successfully for that package
- AND Bun trust verification fails afterward
- WHEN Quantex evaluates the managed install outcome
- THEN Quantex does not run `bun remove -g` for that already-present package
- AND it still reports the Bun install attempt as failed
