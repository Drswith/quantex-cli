...
`info`, `resolve`, and `list` through in-repo Core read ports, and by launching
CLI `exec` / shortcut through an in-repo Core execution engine. They MUST state
that the published `quantex-core` SDK does not gain methods from this CLI
promotion, that package/binary/state identities remain v1-compatible, and that
removal of retained legacy escape routes remains subject to the full soak plus
a separately approved later-major proposal.
