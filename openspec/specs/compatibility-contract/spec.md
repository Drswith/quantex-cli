...
### Requirement: Internalizing provider-binding helpers MUST NOT drift frozen v1 contracts

Moving lifecycle provider-binding resolution and `observeLifecycleProvider` into Core-internal modules MUST NOT change persisted state schema version 2, the lifecycle receipt JSON shape, maintained command aliases, exit-code classes, or v1 `--json` / NDJSON envelopes. Structured output MUST still omit engine and route identifiers. The published `quantex-core` root MUST NOT gain provider-binding or provider-evidence exports as part of this internalization.

#### Scenario: State v2 and receipt JSON stay byte-compatible

- **GIVEN** a valid schema version 2 `state.json` with lifecycle receipts
- **WHEN** Quantex loads or records receipts after the provider-binding move
- **THEN** the schema version remains `2`
- **AND THEN** receipt field names, requiredness, and meanings are unchanged

#### Scenario: Structured CLI output omits engine and route

- **WHEN** a maintained lifecycle command emits JSON or NDJSON after the provider-binding move
- **THEN** the payload does not include selected engine or route identifiers
- **AND THEN** aliases and exit-code classes remain the pre-move v1 contract
...
- **WHEN** a TypeScript consumer imports `quantex-core` or `src/core/index.ts`
- **THEN** the runtime export remains `createQuantex`
- **AND THEN** provider-binding types and `observeLifecycleProvider` are not added to that public surface

### Requirement: Internalizing remaining lifecycle engines MUST NOT drift frozen v1 contracts

Moving agent-observation, update-planner, agent-execution, and uninstall-postcondition into Core-internal modules MUST NOT change persisted state schema version 2, the lifecycle receipt JSON shape, maintained command aliases, exit-code classes, or v1 `--json` / NDJSON envelopes. Structured output MUST still omit engine and route identifiers. The published `quantex-core` root MUST NOT gain those engine exports as part of this internalization.

#### Scenario: State v2 and receipt JSON stay byte-compatible

- **GIVEN** a valid schema version 2 `state.json` with lifecycle receipts
- **WHEN** Quantex loads or records receipts after the engines move
- **THEN** the schema version remains `2`
- **AND THEN** receipt field names, requiredness, and meanings are unchanged

#### Scenario: Structured CLI output omits engine and route

- **WHEN** a maintained lifecycle command emits JSON or NDJSON after the engines move
- **THEN** the payload does not include selected engine or route identifiers
- **AND THEN** aliases and exit-code classes remain the pre-move v1 contract
...
- **WHEN** a TypeScript consumer imports `quantex-core` or `src/core/index.ts`
- **THEN** the runtime export remains `createQuantex`
- **AND THEN** observation, planner, execution-preflight, and uninstall-postcondition helpers are not added to that public surface
...
Moving Core implementation from root `src/core` into `packages/core/src`, relocating package-manager from `src/package-manager` into `packages/core/src/package-manager`, and relocating providers from `src/providers` into `packages/core/src/providers`, MUST NOT change persisted state schema version 2, the lifecycle receipt JSON shape, maintained command aliases, exit-code classes, or v1 `--json` / NDJSON envelopes. Structured output MUST still omit engine and route identifiers. The published `quantex-core` root MUST NOT gain new methods, lifecycle-model exports, package-manager exports, provider exports, or package subpaths as part of this relocation.

#### Scenario: State v2 and receipt JSON stay byte-compatible

- **GIVEN** a valid schema version 2 `state.json` with lifecycle receipts
- **WHEN** Quantex loads or records receipts after the physical Core source move, package-manager relocation, or providers relocation
- **THEN** the schema version remains `2`
- **AND THEN** receipt field names, requiredness, and meanings are unchanged

#### Scenario: Structured CLI output omits engine and route

- **WHEN** a maintained lifecycle command emits JSON or NDJSON after the physical Core source move, package-manager relocation, or providers relocation
- **THEN** the payload does not include selected engine or route identifiers
- **AND THEN** aliases and exit-code classes remain the pre-move v1 contract
...
- **AND THEN** lifecycle engines, receipts, package-manager, providers, and route identifiers are not added to that public surface

#### Scenario: CLI packaged declaration snapshot retargets relocated Core paths

- **WHEN** the CLI package `dist/index.d.mts` is rebuilt after package-manager or providers move under `packages/core/src`
...
- **AND THEN** the snapshot MUST NOT add new exported values, package-manager or providers as a public SDK method, or engine/route identifiers
