...
describe('legacy/Core read differential gate', () => {
  ...
  it('matches provider timeout projection without turning it into a new CLI-level timeout', async () => {
    ...
    const legacy = await createProductionLifecycleObservationService(operationContext(20), {
      resolveLatestVersion: false,
    }).resolveAgentObservation(AGENT_NAME)
    const core = await createCoreService(configDir, operationContext(20)).resolveAgentObservation(AGENT_NAME)
    ...
    const [fullRegistrySdk, slimRegistrySdk] = await Promise.all([
      createSdkClient(configDir, true).inspect(AGENT_NAME, { timeoutMs: 20 }),
      createSdkClient(configDir, false).inspect(AGENT_NAME, { timeoutMs: 20 }),
    ])
