...
describe('schemaCommand', () => {
  ...
  it('returns the doctor schema in json mode', async () => {
    ...
    expect(payload.data.commands[0].dataSchema.properties.installers.properties.deno).toEqual({ type: 'boolean' })
    expect(payload.data.commands[0].dataSchema.properties.installers.properties.mise).toEqual({ type: 'boolean' })
    ...
    expect(payload.data.commands[0].dataSchema.properties.installers.required).toContain('deno')
    expect(payload.data.commands[0].dataSchema.properties.installers.required).toContain('mise')
  ...
  it('returns the capabilities schema with managed installer availability in json mode', async () => {
    ...
    expect(installers.properties.deno.properties.available).toEqual({ type: 'boolean' })
    expect(installers.properties.deno.properties.reason).toEqual({ type: 'string' })
    expect(installers.properties.mise.properties.available).toEqual({ type: 'boolean' })
    expect(installers.properties.mise.properties.reason).toEqual({ type: 'string' })
    ...
    expect(installers.required).toContain('deno')
    expect(installers.required).toContain('mise')
