...
describe('workflow classification integration', () => {
  ...
  it('runs quick real-agent canaries on relevant PRs and full coverage on manual dispatch', () => {
    ...
    expect(agentCanaryWorkflow).toContain('PATH=$isolated_path')
    expect(agentCanaryWorkflow).toContain("QTX_ISOLATION_AGENTS\" == 'claude'")
    expect(agentCanaryWorkflow).toContain('Claude clean-lifecycle PATH still contains an unmanaged executable')
    ...
    expect(agentCanaryWorkflow).toContain('command -v python3')
