...
The SDK does not publish `update`, `uninstall`, `run`, or `doctor`. In the 1.12 CLI slices, mutation commands execute through in-repo Core engines, CLI `inspect` / `info` / `resolve` / `list` observe through in-repo Core read ports, CLI `exec` / shortcut launch through an in-repo Core execution engine (with authorized `--install` missing-agent mutation sharing Core install/ensure), and CLI `doctor` diagnoses through an in-repo Core diagnosis engine, while those unpublished methods remain absent from this package's public method surface. State schema version 2 remains frozen throughout 1.x. CLI `install` / `ensure` apply through in-repo Core only; `--dry-run` uses Core preview while preserving the frozen dry-run plan; the former whole-invocation legacy apply escape is retired. Engine selection occurs before each invocation, with no fallback after mutation side effects begin.
