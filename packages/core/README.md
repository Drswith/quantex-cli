...
The SDK does not publish `update`, `uninstall`, or `run`. In the 1.12 CLI slice those commands execute through in-repo Core engines while remaining absent from this package's public method surface. State schema version 2 remains frozen throughout 1.x. Engine selection occurs before each invocation, with no fallback after mutation side effects begin; install/ensure rollback continues to use the whole-invocation legacy route.
