...
The package is ESM-only and supports Node.js 20 or newer. Bun can consume the same ESM entry point. Core returns typed results and does not prompt, print, call `process.exit`, or own CLI exit-code policy. `install` and `ensure` default to apply mode; pass `{ mode: 'preview' }` for a side-effect-free decision. PATH-only external agents are preserved rather than adopted, and failures expose stable phase and side-effect details without provider internals. Prompts, human and JSON/NDJSON presentation, command-line execution, and CLI self-upgrade presentation remain responsibilities of `qtx` / `quantex`.

The SDK does not publish `update`, `uninstall`, `run`, `doctor`, or `upgrade`. In the 1.12 CLI slices, mutation commands execute through in-repo Core engines, CLI `inspect` / `info` / `resolve` / `list` observe through in-repo Core read ports, CLI `exec` / shortcut launch through an in-repo Core execution engine (with authorized `--install` missing-agent mutation sharing Core install/ensure), and CLI `doctor` diagnoses through an in-repo Core diagnosis engine. CLI `upgrade` plan/check/apply execute through an in-repo Core self-upgrade engine. Those unpublished methods remain absent from this package's public method surface. State schema version 2 remains frozen throughout 1.x. CLI `install` / `ensure` apply through in-repo Core only; `--dry-run` uses Core preview while preserving the frozen dry-run plan; the former whole-invocation legacy apply escape is retired. Engine selection occurs before each invocation, with no fallback after mutation side effects begin.
