...
- Agent-executable work will move toward explicit task contracts in `autonomy/tasks/` backed by `autonomy/queue.md`.
...
- Added the project memory structure under `docs/`, `autonomy/`, and `openspec/`.
- Recorded the policy decision in `docs/adr/0001-agent-native-project-memory.md`.
- Seeded the autonomy queue with the first three migration tasks.
