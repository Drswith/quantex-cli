...
2. **Honest gates.** Required merge checks are exactly the jobs that really run on every PR: `lint`, `governance`, `test (ubuntu-latest)`, `test (windows-latest)`, `test (macos-latest)`. Windows tests now run on product-impacting PRs. `sandbox-tests` is explicitly advisory and never required. `classify` computes context once; every downstream job consumes its outputs. *(Amended 2026-09-02: `test (windows-latest)` remains in `ci.yml` on the product matrix and stays visible in Checks, but it is no longer a required merge gate. The job uses `continue-on-error` so a Windows failure does not fail `ci.yml` or block merge/release tagging. Required contexts are `lint`, `governance`, `test (ubuntu-latest)`, and `test (macos-latest)`. See the `code-quality-tooling` and `release-workflow` specs.)* *(Amended 2026-09-02: for `release-please--branches--*` pull requests, `governance` runs `ci:release-pr-policy` and skips human PR body/commit-policy heuristics so automated Release PRs are not blocked by contributor-oriented text checks. See the `release-workflow` and `code-quality-tooling` specs.)*
...
- Branch rulesets on `main` must require `lint`, `governance`, `test (ubuntu-latest)`, and `test (macos-latest)`; remove `test (windows-latest)` and `sandbox-tests` from required contexts. Windows still runs inside `ci.yml` as advisory signal. The `release-workflow` spec carries this contract.
