...
兼容过渡延续到 1.12：1.2 引入只读 Core，1.3 新增经过 gate 的变更方法组，1.4 让 Core 成为 CLI `install` 与 `ensure` 的 apply 默认引擎，1.12 将 CLI `update` 与 `uninstall` 迁入同一套仓内 Core 引擎，让 CLI `inspect`、`info`、`resolve` 与 `list` 继续走 Core 只读观察，让 CLI `exec` / shortcut 走 Core 执行引擎，并让 CLI `doctor` 走 Core 诊断且不扩展公开 SDK。CLI `install` 与 `ensure` 的 apply 路径已变为 Core-only：`--dry-run` 继续走已冻结的 v1 规划路径且不会产生生命周期副作用，原先的 `QUANTEX_INSTALLATION_ENGINE=legacy` whole-invocation apply 逃生通道已退役。持久化 state schema 在整个 1.x 中保持为 version 2。引擎必须在单次调用前选定，变更操作一旦开始产生副作用就绝不会切换引擎重试。删除其他受维护的 v1 surface 仍须在后续 major 版本另行提出并批准提案。
