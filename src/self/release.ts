...
export function getBinaryReleaseAssetName(executablePath: string = process.execPath): string | undefined {
  ...
  if (executableName.endsWith('.exe')) return 'quantex-windows-x64.exe'

  if (process.platform === 'darwin') return process.arch === 'arm64' ? 'quantex-darwin-arm64' : 'quantex-darwin-x64'

  if (process.platform === 'linux') return process.arch === 'arm64' ? 'quantex-linux-arm64' : 'quantex-linux-x64'
...
export function getBinaryReleaseDownloadUrl(executablePath: string = process.execPath): string | undefined {
  ...
  if (!assetName || !BUILD_REPOSITORY_URL) return undefined
...
export function getBinaryReleaseChecksumUrl(): string | undefined {
  if (!BUILD_REPOSITORY_URL) return undefined
...
export function getSelfUpdateChannel(
  ...
  if (env.QUANTEX_UPDATE_CHANNEL === 'beta') return 'beta'

  if (env.QUANTEX_UPDATE_CHANNEL === 'stable') return 'stable'
...
export async function fetchBinaryReleaseChecksum(assetName: string): Promise<string> {
  ...
  if (!checksumUrl) throw new Error('No checksum file is available for the current release source.')

  const checksumContents = await fetchTextWithCache(checksumUrl, 'self:checksum')
  if (!checksumContents) throw new Error('Failed to download checksum file.')

  const checksum = parseBinaryReleaseChecksum(checksumContents, assetName)

  if (!checksum) throw new Error(`No checksum entry was found for ${assetName}.`)
...
export async function fetchGitHubReleaseSummary(channel: SelfUpdateChannel): Promise<GitHubReleaseSummary> {
  ...
  if (!release) throw new Error(`No GitHub release was found for the ${channel} channel.`)
...
export function parseBinaryReleaseChecksum(contents: string, assetName: string): string | undefined {
  for (const line of contents.split(/\r?\n/)) {
    ...
    if (checksum?.length === 64 && filename === assetName) return checksum.toLowerCase()
  }
...
export function resolveBinaryReleaseAsset(
  ...
  return manifest.assets.find(
    asset =>
      asset.platform === platform && asset.arch === arch && (!currentAssetName || asset.name === currentAssetName),
  )
}
