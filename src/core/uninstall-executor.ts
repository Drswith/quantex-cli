...
const defaultAbsenceDelay = (milliseconds: number) => new Promise<void>(resolve => setTimeout(resolve, milliseconds))

/** Bounded postcondition wait owned by Core uninstall — not a separate lifecycle engine. */
export async function waitForUninstallAbsence(
  ...
  const delay = options.delay ?? defaultAbsenceDelay
