...
Core does not prompt, print, call `process.exit`, or choose CLI exit codes; it returns typed results to its caller. `install` and `ensure` default to apply mode, expose an explicit side-effect-free preview mode, preserve PATH-only external agents, and return typed phase/side-effect failures. The `qtx` / `quantex` CLI remains responsible for prompts, human and JSON/NDJSON presentation, exit-code policy, command-line execution, and Quantex self-upgrade. The published SDK still does not expose `update`, `uninstall`, `run`, or `doctor`; in 1.12 the CLI routes `install`, `ensure`, `update`, and `uninstall` through in-repo Core engines, observes `inspect`, `info`, `resolve`, and `list` through in-repo Core read ports, launches `exec` / shortcut through an in-repo Core execution engine (with authorized `--install` missing-agent mutation sharing the same Core install/ensure engine as CLI install/ensure), and diagnoses `doctor` through an in-repo Core diagnosis engine, while keeping those unpublished SDK methods absent.

The compatibility transition continues through 1.12: 1.2 introduced read-only Core, 1.3 added gated mutation families, 1.4 made Core the CLI apply default for `install` and `ensure`, 1.12 moves CLI `update` and `uninstall` onto the same in-repo Core engines, keeps CLI `inspect`, `info`, `resolve`, and `list` on Core read observation, launches CLI `exec` / shortcut through Core execution with `--install` sharing Core install/ensure, and diagnoses CLI `doctor` through Core without expanding the published SDK. CLI `install` and `ensure` are Core-only for apply: `--dry-run` uses Core preview while preserving the frozen dry-run plan and making no lifecycle side effect, and the former `QUANTEX_INSTALLATION_ENGINE=legacy` whole-invocation apply escape is retired. The persisted state schema remains version 2 throughout 1.x. Engine selection happens before an invocation, and a mutating call never falls back after side effects begin. Removing other maintained v1 surfaces still requires a separately approved later-major proposal.
