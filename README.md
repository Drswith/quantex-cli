...
The compatibility transition continues through 1.12: 1.2 introduced read-only Core, 1.3 added gated mutation families, 1.4 made Core the CLI apply default for `install` and `ensure`, 1.12 moves CLI `update` and `uninstall` onto the same in-repo Core engines, keeps CLI `inspect`, `info`, `resolve`, and `list` on Core read observation, launches CLI `exec` / shortcut through Core execution with `--install` sharing Core install/ensure, and diagnoses CLI `doctor` through Core without expanding the published SDK. CLI `install` and `ensure` are Core-only for apply: `--dry-run` retains the maintained v1 planning path and makes no lifecycle side effect, and the former `QUANTEX_INSTALLATION_ENGINE=legacy` whole-invocation apply escape is retired. The persisted state schema remains version 2 throughout 1.x. Engine selection happens before an invocation, and a mutating call never falls back after side effects begin. Removing other maintained v1 surfaces still requires a separately approved later-major proposal.
