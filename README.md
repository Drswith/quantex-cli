...
Core does not prompt, print, call `process.exit`, or choose CLI exit codes; it returns typed results to its caller. `install` and `ensure` default to apply mode, expose an explicit side-effect-free preview mode, preserve PATH-only external agents, and return typed phase/side-effect failures. The `qtx` / `quantex` CLI remains responsible for prompts, human and JSON/NDJSON presentation, exit-code policy, command-line execution, and Quantex self-upgrade. The SDK still does not publish `update`, `uninstall`, or `run`; their CLI implementations remain on the maintained legacy path.

The compatibility transition is staged across 1.2-1.5: 1.2 introduces read-only Core, 1.3 adds gated mutation families, 1.4 makes Core the CLI apply default for `install` and `ensure`, and 1.5 is the second Core-default soak. Existing v1 `--dry-run` planning stays on the frozen legacy route and makes no lifecycle side effect. The persisted state schema remains version 2 throughout 1.x. Engine selection happens before an invocation, and a mutating call never falls back after side effects begin. During the soak, an operator can retry either promoted command through the retained whole-invocation legacy route: `QUANTEX_INSTALLATION_ENGINE=legacy qtx ensure codex`; use the [rollback rehearsal](./docs/runbooks/core-installation-routing-rollback.md) to collect evidence and return to the Core default. Removing maintained v1 surfaces requires both Core-default stable minors, at least 90 days from default enablement, and a separately approved later-major proposal.
